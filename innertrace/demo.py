...
from .resources import get_template_text
from .tracing import (
    Tracer,
...
from .tracing.projections import list_runs, timeline_view
...
class DemoResult:
    """Paths and final value produced by the offline demo."""
...
def _write_html_demo(run_id: str, events_path: Path, html_path: Path) -> None:
    """Render the bundled timeline template from real projection output."""
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    """Run the installed demo and return a process-style status code."""
    ...
    try:
        ...
        print(f"InnerTrace demo failed: {error}", file=sys.stderr)
        return 1
    ...
    print(f"  innertrace --events-path {result.events_path} timeline --last")
    return 0
